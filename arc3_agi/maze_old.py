...
from numpy import argwhere, array, int32, ones, uint8, uint32, zeros
...
class Maze(Environment2DGrid):
    ...
    def __init__(self, side_length_bits: int = 6, seed: Optional[int] = None) -> None:
        ...
        self.add_layer(self.LKE YS.WALL, mutable=False)
...
class MazeAutomaton(Automaton, radius=1, environment=Maze()):
    ...
    def start_position(self) -> None:
        ...
        self.y, self.x = int(pos[0]), int(pos[1])
        self.path.add((self.y, self.x))

    def tick(self) -> None:
        """Perform one tick of the automaton's behavior."""
        super().tick()  # Get action from genetic code and update position/orientation.
        if (path := (self.y, self.x)) in self.path:
            self.backtracks += 1  # Increment backtrack count if revisiting a cell.
        else:
            self.path.add(path)  # Track visited cells for potential fitness use.
        if self.goal_layer[self.y, self.x] == 1:
            ...
            self.path.clear()  # Clear visited path history for the new run.
